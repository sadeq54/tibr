...
- No AdSense is integrated yet; `NEXT_PUBLIC_ADSENSE_CLIENT` is a placeholder. XM affiliate is the only monetization: `AdSlot`, `AffiliateBanner`, `TradeGoldCta`. Every XM link must go through `xmClickUrl(id, lang, tag)` (sub-ID `t=` for attribution), creatives follow the reader's locale, and nothing XM renders where `xmAllowed(countrySlug)` is false (`lib/xm-banners.ts`). Keep the risk warning next to every CTA.
